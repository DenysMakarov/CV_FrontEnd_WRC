export const BUTTONS_NAV = [
    {title: 'Home', path: '/', num: '01'},
    {title: 'Events', path: '/events', num: '02'},
    {title: 'Tickets', path: '/tickets', num: '03'},
    {title: 'Portfolio', path: '/portfolio', num: '04'},
    {title: 'Contacts', path: '/contacts', num: '05'},
    {title: 'Login', path: '/login', num: '06'},
]



/*
* TO DO DELETE AFTER ADD TO BACKEND
* eventInfo JUST LIKE TEMPLATE FOR EVENT COMPONENT WHICH HAS NOT REDEVELOPED YET
* ALL EVENT IS BEEN GETTING FROM REAL DB
* */

export const MOCK_EVENT_INFO = [
    {
        imgPath: "url(img/rally_c3_4.jpg)",
        numberId: 1,
        titleDesc: "I`am created a unique visual system and strategy across the wide existing spectrum of...",
        // textCenter: "",
        title: "FIA WRC",
        date: "18 July, 2021",
        month: 7,
        year: 2020,
        place: "OAE",
        price: "550 EUR",
        roadDescription: "Known as Australia's premier WRC circuit, Adelaide has always been the scene of drama. One of the most famous moments came in 1986, when Nigel Mansell suffered an impressive left rear tire explosion just 19 laps to go, stripping him of his world title that year. On the track in Adelaide, it was always difficult to find the rhythm in, as there was literally a little bit of everything.",
        mapImg: "url(img/rally_c3_2.jpg)",
        pilotImg_1: "url(img/pilot_1.png)",
        pilotName_1: "Izaac Malsiwicz",
        pilotAge_1: 35,
        pilotExperience_1: 15,
        pilotAwards_1: "Champion of the Europe",
        pilotDesc_1: "Nullam viverra consecte quisque cursus et, porttitor ountur magni dolores eos qui ratione volu incidunt ut labore et dolore magnam aliquam quaerat voluptate quisque cursus et, porttitor ountur magni dolores eos qui ratione.",
        pilotImg_2: "url(img/pilot_2.png)",
        pilotName_2: "Mark Peterson",
        pilotAge_2: 44,
        pilotExperience_2: 21,
        pilotAwards_2: "Champion of the World",
        pilotDesc_2: "Nullam viverra consecte quisque cursus et, porttitor ountur magni dolores eos qui ratione volu incidunt ut labore et dolore magnam aliquam quaerat voluptate quisque cursus et, porttitor ountur magni dolores eos qui ratione.",

    }, {
        imgPath: "url(img/Dakar_rally_toyo.jpg)",
        numberId: 2,
        titleDesc: "I`am created a unique visual system and strategy across the wide existing spectrum of...",
        // textCenter: "",
        title: "DAKAR RALLY",
        date: "21 March, 2021",
        month: 3,
        year: 2021,
        place: "France",
        price: "400 EUR",
        roadDescription: "It is no coincidence that such names as Ayrton Senna and Michael Schumacher were included in the list of winners in Macau. There are not many places in the world that make Monaco look less difficult, but Macau is one of them.",
        mapImg: "url(img/rally_subaru_1.jpg)",
        pilotImg_1: "url(img/pilot_1.png)",
        pilotName_1: "Izaac Malsiwicz",
        pilotAge_1: 35,
        pilotExperience_1: 15,
        pilotAwards_1: "Champion of the Europe",
        pilotDesc_1: "Nullam viverra consecte quisque cursus et, porttitor ountur magni dolores eos qui ratione volu incidunt ut labore et dolore magnam aliquam quaerat voluptate quisque cursus et, porttitor ountur magni dolores eos qui ratione.",
        pilotImg_2: "url(img/pilot_2.png)",
        pilotName_2: "Mark Peterson",
        pilotAge_2: 44,
        pilotExperience_2: 21,
        pilotAwards_2: "Champion of the World",
        pilotDesc_2: "Nullam viverra consecte quisque cursus et, porttitor ountur magni dolores eos qui ratione volu incidunt ut labore et dolore magnam aliquam quaerat voluptate quisque cursus et, porttitor ountur magni dolores eos qui ratione.",
    }, {
        imgPath: "url(img/Bamaco.jpg)",
        numberId: 3,
        titleDesc: "I`am created a unique visual system and strategy across the wide existing spectrum of...",
        // textCenter: "",
        title: "BAMAKO RALLY",
        date: "11 June, 2021",
        month: 6,
        year: 2021,
        place: "Hungary",
        price: "350 EUR",
        roadDescription: "It is quite possibly the most dangerous track that racers could ever dream of, which requires incredible insane courage, similar to that required to participate in a bullfight, so it comes as no surprise that the track was closed in 1977 ... However, being part of the street, the track still exists and some of the roads are still used today for the Targa Florio rally.",
        mapImg: "url(img/Bamaco_2.jpg)",
        pilotImg_1: "url(img/pilot_1.png)",
        pilotName_1: "Izaac Malsiwicz",
        pilotAge_1: 35,
        pilotExperience_1: 15,
        pilotAwards_1: "Champion of the Europe",
        pilotDesc_1: "Nullam viverra consecte quisque cursus et, porttitor ountur magni dolores eos qui ratione volu incidunt ut labore et dolore magnam aliquam quaerat voluptate quisque cursus et, porttitor ountur magni dolores eos qui ratione.",
        pilotImg_2: "url(img/pilot_2.png)",
        pilotName_2: "Mark Peterson",
        pilotAge_2: 44,
        pilotExperience_2: 21,
        pilotAwards_2: "Champion of the World",
        pilotDesc_2: "Nullam viverra consecte quisque cursus et, porttitor ountur magni dolores eos qui ratione volu incidunt ut labore et dolore magnam aliquam quaerat voluptate quisque cursus et, porttitor ountur magni dolores eos qui ratione.",

    }, {
        imgPath: "url(img/f1_maclaren_1.jpg)",
        numberId: 4,
        titleDesc: "I`am created a unique visual system and strategy across the wide existing spectrum of...",
        // textCenter: "",
        title: "GRAND PRI F1",
        date: "1 May, 2021",
        month: 5,
        year: 2021,
        place: "Monaco",
        price: "1050 EUR",
        roadDescription: "The great-grandmother of all street trails is, of course, the Monaco circuit. This track was opened in 1929. The track was designed by Anthony Noges, who has gone down in history because the last turn was named after him. To get an idea of just how crazy the Monaco circuit is, just watch this stunning drive. This video has not been accelerated and is streaming in real time.",
        mapImg: "url(img/f1.jpg)",
        pilotImg_1: "url(img/pilot_1.png)",
        pilotName_1: "Izaac Malsiwicz",
        pilotAge_1: 35,
        pilotExperience_1: 15,
        pilotAwards_1: "Champion of the Europe",
        pilotDesc_1: "Nullam viverra consecte quisque cursus et, porttitor ountur magni dolores eos qui ratione volu incidunt ut labore et dolore magnam aliquam quaerat voluptate quisque cursus et, porttitor ountur magni dolores eos qui ratione.",
        pilotImg_2: "url(img/pilot_2.png)",
        pilotName_2: "Mark Peterson",
        pilotAge_2: 44,
        pilotExperience_2: 21,
        pilotAwards_2: "Champion of the World",
        pilotDesc_2: "Nullam viverra consecte quisque cursus et, porttitor ountur magni dolores eos qui ratione volu incidunt ut labore et dolore magnam aliquam quaerat voluptate quisque cursus et, porttitor ountur magni dolores eos qui ratione.",
    }, {
        imgPath: "url(img/f1_maclaren_2.jpg)",
        numberId: 5,
        titleDesc: "I`am created a unique visual system and strategy across the wide existing spectrum of...",
        // textCenter: "",
        title: "OTHER EVENTS",
        date: "soon...",
        place: "have unknown yet",
        month: "have unknown yet",
        price: "0 EUR",
        roadDescription: "Sorry, we have not information yet.",
        mapImg: "url(img/rally_ford_3.jpg)",
    },
]


export const PORTFOLIO_DATA = {
    portfolio: [
        {
            nameOfWork: "Game",
            link: "https://denysmakarov.github.io/New_CV_Game/dist/index.html",
        },{
            nameOfWork: "Magic Slider",
            link: "http://www.codester.com/index.php?url=items/preview/20183/magicslider-javascript-css-html",

        },{
            nameOfWork: "Sneakers Store",
            link: "https://denysmakarov.github.io/New_CV_SneakersShop/dist/index.html",

        },{
            nameOfWork: "Restaurant",
            link: "https://denysmakarov.github.io/New_CV_restaurant/dist/",
        },
    ]
}
